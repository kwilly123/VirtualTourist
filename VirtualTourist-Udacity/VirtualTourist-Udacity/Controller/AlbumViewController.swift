...
class AlbumViewController: UIViewController {
    ...
    func downloadPhotos() {
        FlickrClient.searchPhotos(latitude: pinSelected!.latitude, longitude: pinSelected!.longitude, totalPages: 3) { (result, error) in
...
extension AlbumViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if (photoImages.count > 0){
            DispatchQueue.main.async {
                collectionCell?.reloadCellWithImage(imageFetched: self.photoImages[indexPath.row])
            }
        } else {
            DispatchQueue.main.async {
